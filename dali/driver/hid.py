...
class tridonic(hid):
    ...
    async def _send_raw(self, command):
        ...
        async with self._command_semaphore:
            ...
            while outstanding_transmissions or response is None:
                ...
                if len(messages) == 0:
                    try:
                        ...
                        self._log.debug(f"_send_raw got event {seq=}")
                    except TimeoutError:
                        self._log.error(f"_send_raw lockup detected, no event received {seq=}")
                        raise
                    event.clear()
            ...
            self._log.debug(f"_send_raw deleting _outstanding {seq=}")
    ...
    def _handle_read(self, data):
        ...
        if data[0] == self._MODE_INFO:
            ...
            if seq in self._outstanding:
                self._log.debug(f"_handle_read: signaling for {seq=}")
                event, messages = self._outstanding[seq]
                messages.append(data)
                event.set()
                del event, messages
            else:
                self._log.debug(f"_handle_read: nothing to do for {seq=}")
